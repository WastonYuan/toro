...
type BohmShell struct {}

func (t BohmShell) GetName() string {
	return "Bohm"
}

func (t BohmShell) Run(opss [](*(t_txn.OPS)), c int, d int) (int, int) {

	bohm := NewBOHM(2, false) // second parameter for debug!
	...
	for i:=0; i < t_count; i++ {
		...
		go func(txn_id int) {
			...
			txns[txn_id] = bohm.NewTXN(txn_id, ops.ReadWriteMap(true))
