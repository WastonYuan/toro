...
func main() {
	...
	for i := 0 ; i < len(core_l); i++ {
		fmt.Printf("%v\t%v\n", core_l[i], bohm.YCSB(core_l[i], 0.001, 0.2, 100, core_l[i]).GetString())
	}
	fmt.Printf("TPCC High Conflict:\nthread\ttps\ttime\twrite_conflict\tread_conflict\n")
	for i := 0 ; i < len(core_l); i++ {
		fmt.Printf("%v\t%v\n", core_l[i], bohm.YCSB(core_l[i], 0.01, 0.5, 100, core_l[i]).GetString())
	}
	...
	for i := 0 ; i < len(skew_l); i++ {
		fmt.Printf("%v\t%v\n", skew_l[i], bohm.YCSB(4, skew_l[i], 0.9, 100, 2).GetString())
	}
	fmt.Printf("YCSB Read Intensive:\nskew\ttps\ttime\twrite_conflict\tread_conflict\n")
	for i := 0 ; i < len(skew_l); i++ {
		fmt.Printf("%v\t%v\n", skew_l[i], bohm.YCSB(4, skew_l[i], 0.1, 100, 2).GetString())
	}
